// node --experimental-strip-types scripts/check-activity-monitoring.mjs
import { classifyInvites, reconcileWinnerSnapshot, partnerRelanceState, partnerRelanceRollup, clientRelanceState, isSilentRequest, isAwaitingChoice, buildTimeline, kpis, partnerPerf } from "../src/lib/activity-monitoring.ts";
...
// reconcileWinnerSnapshot : reporte le devis gagnant first-come sur l'invite du prestataire.
{
  const legacy = [
...
// partnerRelanceState (demande ouverte 'sent')
{
  ...
  ok("relance prestataire due dans Xh (<24h)", dueIn.kind === "dueInMs" && dueIn.ms > 18 * H && dueIn.ms < 20 * H);
