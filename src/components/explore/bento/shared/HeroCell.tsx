...
export function HeroCell({
  ...
  return (
    <div className={`relative flex flex-col justify-end overflow-hidden rounded-2xl bg-sea p-4 ${className}`}>
      {photo ? (
        <Image src={photo} alt={name} fill priority sizes="(max-width:768px) 100vw, 66vw" className="object-cover" />
      ) : (
        <div className="absolute inset-0 bg-gradient-to-br from-sea to-sea-light" />
      )}
      ...
      <div className="pointer-events-none relative z-10">
        <span className="mb-1.5 inline-block rounded-full bg-sea/85 px-2.5 py-1 text-[10px] font-semibold uppercase tracking-wider text-white backdrop-blur-sm">
          {tag}
        </span>
