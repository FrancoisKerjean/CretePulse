// Logique PURE du cockpit de monitoring /admin/activities (pattern activity-admin.ts /
// activity-quotes.ts) : classification des invites, état des relances, timeline, KPI,
// perf prestataire. Zéro I/O. Node-safe : importable par scripts/check-activity-monitoring.mjs.
// Réutilise activity-quotes.ts (jamais réécrit).
import { partnerNeedsRelance, clientNeedsRelance } from "./activity-quotes.ts";
...
/**
 * Réconcilie le snapshot gagnant du modèle first-come (`activity_requests.quoted_*`) dans les invites.
 * Les demandes d'avant le multi-devis portent le devis gagnant sur la DEMANDE, mais l'invite du
 * prestataire reste `invited` sans prix : sans réconciliation, le gagnant apparaît à tort « silencieux »
 * et les KPI comptent 0 devis. On reporte le devis snapshoté sur l'invite du prestataire gagnant.
...
/** État de relance prestataire d'UNE invite (silencieuse). Réutilise partnerNeedsRelance. */
...
/** Rollup relances prestataire d'une demande : invités (status 'invited'), relancés, silencieux. */
...
export function buildTimeline(
  ...
  if (invites.length > 0) {
    ...
    ev.push({ at: firstInvite, label: `${invites.length} prestataire(s) invité(s)` });
  }
  ...
  for (const i of invites) {
    if (i.relanced_at) ev.push({ at: i.relanced_at, label: `Relance prestataire (${i.partner_name})` });
...
/** Perf prestataire enrichie (au-delà de partnerStats.won). invitesByPartner = invites du prestataire. */
