// Back-office Activities Direct. Server component : auth (query/cookie),
...
import { kpis, reconcileWinnerSnapshot } from "@/lib/activity-monitoring";
import type { MonitorInvite } from "@/lib/activity-monitoring";
...
export default async function ActivityAdminPage({
  ...
  if (key) {
    if (await isActivityAdmin(key)) redirect(`/admin/activities/auth?key=${encodeURIComponent(key)}`);
    notFound();
  }
  ...
  let invitesFull: {
    ...
    quote_price: number | null; quote_currency: string | null; quote_details: string | null;
    created_at: string; quoted_at: string | null; declined_at: string | null; relanced_at: string | null;
    activity_partners?: { name?: string };
  }[] = [];
  let loadError: string | null = null;
  try {
    const [reqRes, partRes, invRes, invFullRes] = await Promise.all([
      supabase.from("activity_requests").select("*").order("created_at", { ascending: false }).limit(1000),
      supabase.from("activity_partners").select("*").order("id"),
      supabase.from("activity_quote_invites").select("request_id, partner_id"),
      supabase.from("activity_quote_invites").select(
        "id, request_id, partner_id, status, quote_price, quote_currency, quote_details, created_at, quoted_at, declined_at, relanced_at, activity_partners(name)"
      ),
    ]);
  ...
  for (const r of invitesFull) {
    ...
    list.push({
      ...
      partner_name: r.activity_partners?.name ?? "Prestataire",
      status: r.status, quote_price: r.quote_price, quote_currency: r.quote_currency,
      quote_details: r.quote_details, created_at: r.created_at,
      quoted_at: r.quoted_at, declined_at: r.declined_at, relanced_at: r.relanced_at,
    });
    monitorByRequest.set(r.request_id, list);
  }

  // Réconcilie le snapshot gagnant first-come (activity_requests.quoted_*) sur l'invite du prestataire :
  ...
  // monitorByPartner construit UNE fois à partir de monitorByRequest (pas de N+1).
  ...
  // Calcul des KPI sur fenêtres 7j / 30j.
  ...
  return (
    <main className="mx-auto max-w-6xl px-4 py-8">
      <h1 className="font-heading text-2xl font-extrabold">Activities Direct · admin</h1>

      {loadError ? (
        <p className="mt-4 rounded-xl border border-terracotta bg-terracotta-faint p-4 text-sm">
          Erreur de lecture : {loadError} (migration appliquée ? DB joignable ?)
        </p>
      ) : null}
      ...
      <section className="mt-5 flex flex-wrap items-stretch gap-3">
        ...
        <div className="flex flex-wrap content-center items-center gap-1.5 text-sm">
          {([["sent", s.byStatus.sent ?? 0], ["quoted", s.byStatus.quoted ?? 0], ["accepted", s.byStatus.accepted ?? 0],
            ...
            <a key={st} href={`/admin/activities?status=${st}`}
      ...
      <nav className="mt-6 flex gap-2">
        <a href="/admin/activities" className={`rounded-full px-4 py-1.5 text-sm font-bold no-underline ${tab === "requests" ? "bg-sea text-white" : "border border-border bg-white text-text"}`}>
          Demandes ({requests.length})
        </a>
        <a href="/admin/activities?tab=partners" className={`rounded-full px-4 py-1.5 text-sm font-bold no-underline ${tab === "partners" ? "bg-sea text-white" : "border border-border bg-white text-text"}`}>
          Prestataires ({partners.length})
        </a>
      </nav>
