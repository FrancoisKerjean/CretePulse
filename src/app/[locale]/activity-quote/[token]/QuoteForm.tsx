...
import { ACTIVITY_INCLUSION_KEYS, ACTIVITY_INCLUSION_LABELS_PARTNER } from "@/lib/activity-inclusions";

// Copy i18n du bouton de désistement prestataire (le reste du form reste EN :
// les prestataires sont des pros locaux, l'anglais suffit).
const DECLINE_COPY: Record<string, { link: string; sending: string; done: string }> = {
  en: { link: "I can't take this one", sending: "Sending…", done: "Noted, thank you." },
...
// Formulaire de saisie du prix par le prestataire activité. Poste le prix,
// le titre d'offre et les inclusions au jeton (en clair depuis l'URL) à l'API.
export function QuoteForm({ token, locale = "en" }: { token: string; locale?: string }) {
  ...
  async function submit(e: React.FormEvent) {
    ...
    setState("sending");
    try {
      const res = await fetch("/api/activities/quote", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ token, price: value, details: details.trim() || null, inclusions }),
      });
  ...
  async function declineRequest() {
    setDecline("sending");
    try {
      const res = await fetch("/api/activities/quote?decline=1", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  return (
    <form onSubmit={submit} style={{ display: "flex", flexDirection: "column", gap: 12 }}>
      <label style={{ fontSize: 14, fontWeight: 600, color: "#0B3954" }}>
        Total price for the whole group (EUR)
      </label>
      ...
      <label style={{ fontSize: 14, fontWeight: 600, color: "#0B3954" }}>Offer title (e.g. Sunset food walk) <span style={{ fontWeight: 400, color: "#94A3B8" }}>(optional)</span></label>
      <input
        type="text" value={details} onChange={(e) => setDetails(e.target.value)}
        placeholder="e.g. Sunset food walk"
        maxLength={120}
        style={{ padding: "12px 14px", fontSize: 16, borderRadius: 10, border: "1px solid #DCE9EE", outline: "none" }}
      />
      <span style={{ fontSize: 14, fontWeight: 600, color: "#0B3954" }}>Included in the price <span style={{ fontWeight: 400, color: "#94A3B8" }}>(optional)</span></span>
      <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
        {ACTIVITY_INCLUSION_KEYS.map((k) => {
          ...
          return (
            <button key={k} type="button"
              ...
              {ACTIVITY_INCLUSION_LABELS_PARTNER[k]}
            </button>
          );
        })}
      </div>
