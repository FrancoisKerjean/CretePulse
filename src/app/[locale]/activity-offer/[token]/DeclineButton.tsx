...
// Bouton « aucune de ces offres ne me convient » (désistement client activité).
// Ferme la demande côté client et coupe les relances, sans email aux prestataires.
export function DeclineButton({ token, label, doneText }: { token: string; label: string; doneText: string }) {
  ...
  async function decline() {
    setState("sending");
    try {
      const res = await fetch("/api/activities/accept?decline=1", {
        method: "POST",
