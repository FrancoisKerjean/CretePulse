...
- **`main` = prod** (Vercel déploie depuis main). Tu NE pousses **JAMAIS** `main` toi-même. Ton geste de fin de chantier s'arrête à `git push origin master` (0 build). La promotion `master → main` est **automatique, 1×/jour à 20h Athens** (GitHub Action `daily-deploy`) : elle embarque tout ce qui a atterri sur `master` dans la journée en **un seul build prod**, ce qui coupe les vagues d'écritures ISR (chaque push main re-génère ~24K pages × 22 langues). Hotfix urgent = exception assumée : `git push origin master:main` manuel, ou bouton « Run workflow » sur l'Action `daily-deploy`.
