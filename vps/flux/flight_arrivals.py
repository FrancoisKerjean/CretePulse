#!/usr/bin/env python3
"""Historise les arrivees ET les departs des aeroports cretois.

HER : tableaux HTML officiels screen.herairport.com (MAJ 15s). Chaque tableau
couvre ~24h glissantes : assign_service_dates date les lignes apres minuit au
jour suivant.
CHQ : JSON officiel Fraport (_jcr_content.arrivals/departures.json), datetimes
complets (pas de fenetre glissante), lu = heure reelle d'atterrissage.

Cron : */10 (4 combinaisons airport x direction).
Upsert par (airport, direction, service_date, flight_no, sched_time).
"""
import json
import sys
import urllib.request

from db import connect
from parsers import (assign_service_dates, parse_arrivals, parse_chq,
                     parse_departures, parse_service_date)

URLS = {
    ("HER", "arrival"): "https://screen.herairport.com/arr2web.php",
    ("HER", "departure"): "https://screen.herairport.com/dep2web.php",
    ("CHQ", "arrival"): "https://www.chq-airport.gr/en/_jcr_content.arrivals.json",
    ("CHQ", "departure"): "https://www.chq-airport.gr/en/_jcr_content.departures.json",
}
UA = "Mozilla/5.0 (compatible; CreteDirectFlux/1.0; +https://crete.direct)"

UPSERT_SQL = """
insert into flux_flight_arrivals
  (airport, direction, service_date, sched_time, flight_no, airline_code, origin, status, belt, landed_at)
values (%(airport)s, %(direction)s, %(service_date)s, %(sched_time)s, %(flight_no)s, %(airline_code)s,
        %(origin)s, %(status)s, %(belt)s,
        coalesce(%(landed_at)s::timestamptz,
                 case when %(status)s = 'Landed' then now() end))
on conflict (airport, direction, service_date, flight_no, sched_time) do update set
  status = coalesce(excluded.status, flux_flight_arrivals.status),
  belt = coalesce(excluded.belt, flux_flight_arrivals.belt),
  last_seen_at = now(),
  landed_at = coalesce(flux_flight_arrivals.landed_at, excluded.landed_at);
"""


def fetch(url):
    req = urllib.request.Request(url, headers={"User-Agent": UA})
    with urllib.request.urlopen(req, timeout=20) as r:
        return r.read().decode("utf-8", errors="replace")


def parse_her(raw, direction):
    day = parse_service_date(raw)
    parse = parse_arrivals if direction == "arrival" else parse_departures
    rows = assign_service_dates(parse(raw), day)
    for row in rows:
        row["landed_at"] = None  # HER : pose au passage du statut 'Landed'
    return rows


def run(airport, direction):
    raw = fetch(URLS[(airport, direction)])
    if airport == "HER":
        rows = parse_her(raw, direction)
    else:
        rows = parse_chq(json.loads(raw), direction)
    for row in rows:
        row["airport"] = airport
        row["direction"] = direction
        if direction == "departure":
            row["origin"] = row.pop("destination")  # meme colonne, sens = autre bout du vol
    conn = connect()
    conn.autocommit = True
    with conn, conn.cursor() as cur:
        for row in rows:
            cur.execute(UPSERT_SQL, row)
    conn.close()
    print(f"{airport} {direction}: {len(rows)} vols upsert")


if __name__ == "__main__":
    try:
        airport = "CHQ" if "--chq" in sys.argv else "HER"
        run(airport, "departure" if "--departures" in sys.argv else "arrival")
    except Exception as exc:  # cron : log et exit != 0
        print(f"flight_arrivals ERROR: {exc}", file=sys.stderr)
        sys.exit(1)
